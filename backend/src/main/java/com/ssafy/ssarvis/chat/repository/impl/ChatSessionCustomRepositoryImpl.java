    ...
    public List<ChatSessionDocument> findDynamicSessionsByType(Long reqUserId, String type,
        ...
        if ("SECRETARY".equals(type)) {
            ...
            criteria.andOperator(
                ...
                Criteria.where("target_user_id").is(reqUserId),
            ...
            if (assistantType != null) {
                criteria.and("assistant_type").is(assistantType);
            } else {
                criteria.and("assistant_type").in(AssistantType.DAILY, AssistantType.STUDY, AssistantType.COUNSEL);
            }
            ...
            criteria.andOperator(
                Criteria.where("target_user_id").is(reqUserId),
                Criteria.where("assistant_type").is(AssistantType.PERSONA)
            );

            if (chatSessionType != null) {
                criteria.and("chat_session_type").is(chatSessionType);
            }
            ...
            criteria.andOperator(
                Criteria.where("target_user_id").is(reqUserId),
                Criteria.where("user_id").ne(reqUserId) // 타인 개입 방어
            );
            ...
            criteria.andOperator(
                Criteria.where("user_id").is(reqUserId),
                Criteria.where("target_user_id").ne(reqUserId) // 타인 개입 방어
            );
