...
| 기술 영역 | 요약 |
| --- | --- |
| 사용자 특성 모사 | 간편한 설문을 통해 사용자의 가치관, 말투, 감정 반응, 대화 습관을 수집하고 이를 사용자 모방 프롬프트로 구조화합니다. |
| 제로샷 음성 복제 | 최근 공개된 Qwen3-TTS 모델의 Voice Cloning을 적용하여 사용자의 음성을 즉시 제로샷으로 모사합니다. |
| RAG 기반 기억 | 사용자의 이전 대화를 벡터 DB에 저장하고 RAG를 통해 정확한 과거 대화 내용에 근거한 응답을 생성합니다. |
| 지속 갱신 페르소나 | 사용자와 AI의 대화가 누적될수록 시스템 프롬프트를 지속적으로 업데이트하여 사용자를 더 잘 모사하도록 개선합니다. |
| TTS 비용 최적화 | 자주 호출되는 음성은 소형 TTS 모델을 훈련해 사용하고, 단일 음성을 증강 및 평가하여 데이터셋을 구축하는 훈련 파이프라인을 구현했습니다. |
